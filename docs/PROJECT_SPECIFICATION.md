...
The operational controller keeps the approved regular strategy cycle unchanged: one automated cycle per eligible US trading day at `09:28 America/New_York`, with broker transmission no earlier than `09:30 America/New_York`. Because strategy signals use completed daily bars from the prior completed US session, the controller now also performs an advance read-only evaluation immediately after the successful `08:30 America/New_York` daily-bar refresh. That advance evaluation writes `reports/preopen_preview_report.json` and sends Telegram details for the planned BUY/SELL orders, including planned quantity and LIMIT price where applicable, while explicitly confirming that broker-submitted orders remain zero. The advance evaluation returns before the order-transmission wait and before `process_order_plan()`. The existing 09:28 strategy cycle then runs exactly as before and remains the authoritative execution path, including its fresh broker/account refresh before the unchanged 09:30 transmission step. If account state changes between the advance evaluation and the regular cycle, the regular cycle remains authoritative. Broker transmission remains gated by the existing PAPER/account, market-hours, liquid-hours, cash/buying-budget, duplicate-order and long-only safety controls.
