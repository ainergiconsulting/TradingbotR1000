...
The operational controller currently evaluates one automated strategy cycle per eligible US trading day at `09:28 America/New_York`, then transmits eligible broker orders no earlier than `09:30 America/New_York`. The strategy signals themselves use completed daily bars from the prior completed US session, so `09:28` is an implementation scheduling choice rather than a strategy-data requirement. The current design computes candidates immediately before the opening bell, then refreshes live account, position and open-order evidence again immediately before broker transmission. Broker transmission remains gated by live IBKR market-hours and liquid-hours evidence and is refused outside liquid hours. The controller records the last completed cycle date to prevent repeated entry cycles for the same trading day. Candidate generation may be decoupled to an earlier post-refresh time in a future operational change without changing strategy inputs, provided the 09:30 broker-state/safety refresh remains authoritative.
