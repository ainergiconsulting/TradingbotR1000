...
The operational controller keeps the approved regular strategy cycle unchanged: one automated cycle per eligible US trading day at `09:28 America/New_York`, with broker transmission no earlier than `09:30 America/New_York`. Because strategy signals use completed daily bars from the just-finished US session, the daily-bar refresh now begins at `16:30 America/New_York`, 30 minutes after the regular close. The clock time is only a first-attempt trigger: correctness is determined by the session date returned by IBKR. If IBKR still reports the prior session, the controller waits five minutes and retries; no preview is accepted until `expected_latest_completed_session` equals the just-completed ET session date. Once confirmed, the bot immediately performs a read-only next-session evaluation, writes `reports/preopen_preview_report.json`, and sends Telegram details for the planned BUY/SELL orders, including planned quantity and LIMIT price where applicable, while explicitly confirming that broker-submitted orders remain zero. A Friday post-close evaluation is therefore labeled for the following eligible session (normally Monday). The advance evaluation returns before the order-transmission wait and before `process_order_plan()`. The existing 09:28 strategy cycle then runs exactly as before and remains the authoritative execution path, including its fresh broker/account refresh before the unchanged 09:30 transmission step. If account state changes between the post-close preview and the regular cycle, the regular cycle remains authoritative. Broker transmission remains gated by the existing PAPER/account, market-hours, liquid-hours, cash/buying-budget, duplicate-order and long-only safety controls.
