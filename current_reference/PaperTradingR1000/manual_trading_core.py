...
class BrokerActionUncertainError(BrokerActionError):
    """Broker submission may have succeeded; reconciliation is required before retry."""

    def __init__(self, message: str, *, order_details: dict | None = None):
...
def _load_mobile_request_state() -> dict:
    try:
        data = json.loads(MOBILE_REQUEST_STATE_FILE.read_text(encoding="utf-8"))
        return data if isinstance(data, dict) else {}
    except FileNotFoundError:
        return {}
    except Exception as error:
        raise ManualControlError(
            f"Mobile request state unavailable: {type(error).__name__}. Trading is fail-closed."
        ) from error


def _write_mobile_request_state(state: dict) -> None:
    MOBILE_REQUEST_STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp = MOBILE_REQUEST_STATE_FILE.with_suffix(MOBILE_REQUEST_STATE_FILE.suffix + ".tmp")
    tmp.write_text(json.dumps(state, indent=2, sort_keys=True, default=str), encoding="utf-8")
    os.replace(tmp, MOBILE_REQUEST_STATE_FILE)


def reserve_mobile_request(request_id: str, action: str, details: dict | None = None) -> dict:
    """Persistently reserve a mobile mutation request before any broker action."""
    request_id = str(request_id or "").strip()
    if not request_id or len(request_id) > 128:
        raise ManualControlError("A valid mobile request ID is required.")
    with _mobile_request_lock:
        state = _load_mobile_request_state()
        existing = state.get(request_id)
        if existing is not None:
            return {"accepted": False, "duplicate": True, "record": existing}
        record = {
        ...
        _write_mobile_request_state(state)
        return {"accepted": True, "duplicate": False, "record": record}


def update_mobile_request(request_id: str, status: str, **details) -> dict:
    """Update a reserved mobile request atomically; unknown IDs are refused."""
    request_id = str(request_id or "").strip()
    with _mobile_request_lock:
        state = _load_mobile_request_state()
        if request_id not in state:
            raise ManualControlError("Mobile request ID is not reserved. Trading is fail-closed.")
        record = state[request_id]
        record["status"] = str(status or "UNKNOWN").upper()
        record["updated_utc"] = _utc_timestamp()
        if details:
...
def get_mobile_request(request_id: str) -> dict | None:
    with _mobile_request_lock:
        return _load_mobile_request_state().get(str(request_id or "").strip())


def mobile_request_requires_reconciliation(request_id: str) -> bool:
    record = get_mobile_request(request_id)
    if not record:
        return False
    return str(record.get("status") or "").upper() in {
        "SUBMITTING",
...
def place_manual_order(
    ...
    try:
        ...
        with order_guard_context as order_guard:
            ...
            # From this point onward the broker may have accepted the order.
            # Any local/API exception must therefore be treated as uncertain,
            # never as a safe failure that the caller may blindly retry.
            try:
                outcome = wait_for_trade_outcome(ib, trade)
                if order_guard is not None:
                    order_guard.mark_submitted(trade)
            except Exception as error:
                identifiers = _order_identifiers(trade)
                uncertain_details = _combine_details(audit_details, identifiers)
                if order_guard is not None:
                    try:
                        order_guard.mark_submitted(trade)
                    except Exception:
                ...
                raise BrokerActionUncertainError(
                    "IBKR order submission may have succeeded, but the final broker "
                    "state could not be established. Do not retry this order until "
                    "broker state has been reconciled.",
                    order_details=uncertain_details,
                ) from error
    except BrokerActionUncertainError:
        raise
    except Exception as error:
        _post_broker_audit(
            "PLACE_ORDER",
            "FAILED_BEFORE_CONFIRMED_SUBMISSION",
            **audit_details,
            error_type=type(error).__name__,
        )
        raise BrokerActionError(
            f"IBKR order placement failed before submission could be confirmed: {type(error).__name__}"
        ) from error
