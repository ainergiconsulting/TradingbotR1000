...
from flex_execution_ledger import record_commission_report
...
def _on_exec_details(trade, fill) -> None:
    ...
    process_execution(
        ...
        price=payload["price"],
        execution_time=payload["date_time"],
...
def run() -> int:
    while True:
        ...
        try:
            ...
            print("Execution monitor connected (read-only polling).", flush=True)
            # Poll because fills generated by the strategy's different IBKR API
            # client are not guaranteed to emit execDetailsEvent to this client.
